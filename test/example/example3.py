...
class AudioPlayer(
	...
	def __init__(
		...
		self.capture_session.setAudioInput(
			self.audio_input
		)
	...
	def update_recording_filename(
		...
		self.recorder.setOutputLocation(
			QUrl.fromLocalFile(
				f'C:/Users/aaaap/Documents/pyloid/pyloid/{filename}'
			)
		)

	def on_recorder_state_changed(
		self,
		state,
	):
		if (
			...
			print(
				f'녹음이 저장되었습니다: recording_{self.recording_counter}.wav'
			)
			self.recording_counter += 1
			self.update_recording_filename()
