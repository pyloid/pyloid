	...
	def get_resize_direction(self, pos):
		if (
			...
			if pos.y() <= margin:
				...
				direction = (
					'bottom' if direction is None else direction + '-bottom'
				)
...
class _BrowserWindow:
			...
			for js_api in self.js_apis:
				if js_api.__class__.__name__ == 'BaseAPI':
					...
					self.channel.registerObject(
						js_api.__class__.__name__, js_api
					)
	...
	def _on_load_finished(self, ok):
		...
		if ok and self.js_apis:
			...
			base_api_init = (
				"window['__PYLOID__'] = channel.objects['__PYLOID__'];\n"
			)
	...
	def open_dev_tools(self):
		...
		self.web_view.page().setDevToolsPage(
			QWebEnginePage(self.web_view.page())
		)
	...
	def set_static_image_splash_screen(
		...
		window.set_static_image_splash_screen(
		    './assets/loading.png', close_on_load=True, stay_on_top=True
		)
		```
		"""
	...
	def set_gif_splash_screen(
		...
		window.set_gif_splash_screen(
		    './assets/loading.gif', close_on_load=True, stay_on_top=True
		)
		```
		"""
...
class BrowserWindow(QObject):
	...
	def set_position_by_anchor(self, anchor: str) -> None:
		...
		"""
		return self.execute_command(
			'set_position_by_anchor', {'anchor': anchor}
		)
	...
	def set_context_menu(self, context_menu: bool) -> None:
		...
		"""
		return self.execute_command(
			'set_context_menu', {'context_menu': context_menu}
		)
	...
	def remove_shortcut(self, key_sequence: str) -> None:
		...
		"""
		return self.execute_command(
			'remove_shortcut', {'key_sequence': key_sequence}
		)
	...
	def invoke(self, event_name: str, data: 'Optional[Dict]' = None) -> None:
		...
		"""
		return self.execute_command(
			'invoke', {'event_name': event_name, 'data': data}
		)
