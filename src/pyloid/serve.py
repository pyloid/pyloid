		...
		try:
			...
			if range_header:
				return await self._handle_range_request(
					file_path, range_header, file_size, content_type, request
				)
			...
			return ZeroCopyFileResponse(
				path=file_path, chunk_size=self.chunk_size, headers=headers
			)

		except Exception as e:
			return web.Response(
				status=500, text=f'Internal Server Error: {str(e)}'
			)

	async def _handle_range_request(
		self, file_path, range_header, file_size, content_type, request
	):
		...
		try:
			...
			return web.Response(
				status=500, text=f'Range Request Error: {str(e)}'
			)
